...
export function arcToCubics(from, segment) {
    const to = segment.to;
    ...
    const numerator = Math.max(0, radiusX ** 2 * radiusY ** 2 - radiusX ** 2 * transformedY ** 2 - radiusY ** 2 * transformedX ** 2);
    ...
    if (segment.sweep === "clockwise" && sweepAngle < 0)
        sweepAngle += Math.PI * 2;
    if (segment.sweep === "counterclockwise" && sweepAngle > 0)
        sweepAngle -= Math.PI * 2;
    const sectionCount = Math.max(1, Math.ceil(Math.abs(sweepAngle) / (Math.PI / 2)));
    const sectionAngle = sweepAngle / sectionCount;
    const pointAndDerivative = (angle) => {
...
function pointOnQuadratic(a, b, c, t) {
    const opposite = 1 - t;
    return {
...
export function flattenCompoundSubpath(subpath, steps = 12) {
    ...
    for (const segment of subpath.segments) {
        if (segment.type === "line") {
            ...
            if (samePoint(current, segment.to))
                continue;
        }
        current = segment.to;
    }
...
export function compoundPathData(subpaths, frame, format) {
    const commands = [];
    for (const subpath of subpaths) {
        ...
        for (const segment of subpath.segments) {
            if (segment.type === "line") {
                ...
                const to = mapPoint(segment.to, frame);
                commands.push(`C ${format(control1.x)} ${format(control1.y)} ${format(control2.x)} ${format(control2.y)} ${format(to.x)} ${format(to.y)}`);
            }
            else {
                const cubics = arcToCubics(current, segment);
                if (cubics.length === 0)
                    throw new Error("Arc start and endpoint must differ");
