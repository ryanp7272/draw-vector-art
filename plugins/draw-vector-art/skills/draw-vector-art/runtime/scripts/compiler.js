...
import { frameCorners, frameMapMatrix, IDENTITY_MATRIX, isIdentityMatrix, matrixPower, mirrorMatrix, multiplyMatrices, transformMatrix, } from "./matrix.js";
import { resolveScene } from "./resolver.js";
...
function addDefinition(collector, key, markup) {
...
function gradientDefinition(id, gradient, frame, scene) {
    const stops = gradient.stops
        .map((stop) => `<stop offset="${number(stop.offset)}%" stop-color="${color(stop.color, scene)}" stop-opacity="${number(stop.opacity)}"/>`)
        .join("");
...
function paintValue(value, scene, renderedId, frame, definitions) {
    if (!value.startsWith("gradient:"))
        return color(value, scene);
    const name = value.slice("gradient:".length);
    const gradient = scene.gradients[name];
    if (!gradient)
        throw new Error(`Unknown gradient reference ${value}`);
...
function shadowDefinition(id, paint, frame, scene) {
    if (!paint.effect)
        return undefined;
    const name = paint.effect.slice("effect:".length);
    const effect = scene.effects[name];
    if (!effect)
        throw new Error(`Unknown effect reference ${paint.effect}`);
...
function paintAttributes(paint, scene, clip = false, renderedId, frame, definitions) {
    if (clip)
        return 'fill="#fff" stroke="none"';
    if (!renderedId || !frame || !definitions)
        throw new Error("Rendered paint requires a resource context");
    const attributes = [
    ...
    if (paint.effect) {
        ...
        if (definition)
            addDefinition(definitions, `effect:${id}`, definition);
...
function primitiveElement(object, frame, scene, definitions, id, paintOverride, clip = false, transform) {
...
function cloneSourceVisible(resolved, graph, trail = new Set()) {
...
function recordDebug(state, context, frame, localMatrix) {
    if (!context.collectDebug || context.clip)
        return;
...
function cloneMatrices(resolved, context) {
    ...
    if (resolved.object.frame.type !== "mirror")
        return { local: beforeMirror };
    ...
    if (isIdentityMatrix(context.extraMatrix) && !resolved.object.transform) {
        ...
        return {
            local,
            legacyTransform: resolved.object.frame.axis === "x"
...
function geometryForClone(resolved, context, state) {
    if (resolved.object.type !== "clone")
        throw new Error(`${resolved.object.id} is not a clone`);
    if (!cloneSourceVisible(resolved, state.graph))
        return "";
    const source = sourcePrimitive(resolved, state.graph);
    if (!isPrimitive(source.object))
    ...
    return primitiveElement(source.object, resolved.frame, state.graph.scene, state.definitions, context.outputId, resolved.object.paint, context.clip, transform);
}
function sourceFor(resolved, state, trail) {
    ...
    if (!source)
        throw new Error(`${resolved.object.type} ${resolved.object.id} references missing source ${resolved.object.source}`);
    const next = new Set(trail);
    next.add(resolved.object.id);
    if (next.has(source.object.id))
        throw new Error(`Reusable source cycle detected at ${source.object.id}`);
    next.add(source.object.id);
    return { source, trail: next };
}
function targetMap(resolved, source) {
...
function childId(parent, object) {
    return parent.copied ? `${parent.outputId}__${object.id}` : object.id;
}
function registerClip(resolved, context, state) {
...
function renderPrimitive(resolved, context, state) {
    if (!isPrimitive(resolved.object))
        throw new Error(`${resolved.object.id} is not a primitive`);
    ...
    return primitiveElement(resolved.object, resolved.frame, state.graph.scene, state.definitions, context.outputId, undefined, context.clip, transform);
}
function renderGroup(resolved, context, state) {
    if (resolved.object.type !== "group")
        throw new Error(`${resolved.object.id} is not a group`);
    const ownTransform = transformMatrix(resolved.object.transform, resolved.frame, resolved.space);
    const localMatrix = multiplyMatrices(context.extraMatrix, ownTransform);
    recordDebug(state, context, resolved.frame, localMatrix);
    ...
    const children = resolved.children
        .map((child) => renderResolved(child, {
...
function renderInstance(resolved, context, state) {
    if (resolved.object.type !== "instance")
        throw new Error(`${resolved.object.id} is not an instance`);
    ...
    const content = renderResolved(source, {
...
function renderRepeater(resolved, context, state) {
    if (resolved.object.type !== "repeater")
        throw new Error(`${resolved.object.id} is not a repeater`);
    const { source, trail } = sourceFor(resolved, state, context.sourceTrail);
    const ownTransform = transformMatrix(resolved.object.transform, resolved.frame, resolved.space);
    const localMatrix = multiplyMatrices(context.extraMatrix, ownTransform);
    recordDebug(state, context, resolved.frame, localMatrix);
    ...
    for (let index = 0; index < resolved.object.count; index += 1) {
        ...
        copies.push(`<g id="${escapeXml(copyId)}" data-source="${escapeXml(source.object.id)}"${matrixAttribute(copyMatrix)}>${content}</g>`);
    }
    return `<g id="${escapeXml(context.outputId)}" data-source="${escapeXml(source.object.id)}"${matrixAttribute(localMatrix)}>${copies.join("")}</g>`;
}
function renderResolved(resolved, context, state) {
    if (!resolved.visible)
        return "";
    if (resolved.object.template && !context.forceTemplate)
        return "";
...
export function compileSvg(scene, options = {}) {
    ...
    const state = {
        graph,
        definitions: { values: [], keys: new Set() },
        debugItems: [],
    };
