...
export function skewXMatrix(degrees) {
    return { a: 1, b: 0, c: Math.tan((degrees * Math.PI) / 180), d: 1, e: 0, f: 0 };
}
export function skewYMatrix(degrees) {
...
export function transformMatrix(transform, frame, space) {
    if (!transform)
...
export function frameMapMatrix(source, target) {
    return [
...
export function mirrorMatrix(frame, axis) {
    const centerX = frame.x + frame.width / 2;
    const centerY = frame.y + frame.height / 2;
    return axis === "x"
        ? multiplyMatrices(translationMatrix(2 * centerX, 0), scaleMatrix(-1, 1))
        : multiplyMatrices(translationMatrix(0, 2 * centerY), scaleMatrix(1, -1));
}
export function frameCorners(frame, matrix = IDENTITY_MATRIX) {
    return [
        { x: frame.x, y: frame.y },
...
export function isIdentityMatrix(matrix, epsilon = 1e-10) {
