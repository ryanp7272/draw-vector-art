...
async function main() {
    ...
    if (parsed.command === "benchmark") {
        ...
        if (!runPath)
            throw new Error("benchmark requires a run JSON path");
        const outputDirectory = parsed.options.get("out");
        if (!outputDirectory)
            throw new Error("benchmark requires --out <directory>");
    ...
    if (!scenePath)
        throw new Error(`${parsed.command} requires a scene JSON path`);
    ...
    const outputDirectory = parsed.options.get("out");
    if (!outputDirectory)
        throw new Error(`${parsed.command} requires --out <directory>`);
    ...
    if (parsed.command === "compare") {
        ...
        if (!reference)
            throw new Error("compare requires --reference <image> or a scene reference");
