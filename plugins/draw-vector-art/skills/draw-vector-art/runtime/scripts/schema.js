...
const PaintColorRefSchema = z
    ...
    HEX_COLOR_PATTERN.test(value), "Use none, a palette reference, a named gradient such as gradient:sunlit, or a hexadecimal color");
...
const GradientStopSchema = z
    ...
    color: z
        .string()
        .refine((value) => PALETTE_REF_PATTERN.test(value) || HEX_COLOR_PATTERN.test(value), "Gradient stops use a palette reference or hexadecimal color"),
    opacity: finiteNumber.min(0).max(1).default(1),
})
    .strict();
...
const ShadowEffectSchema = z
    ...
    color: z
        .string()
        .refine((value) => PALETTE_REF_PATTERN.test(value) || HEX_COLOR_PATTERN.test(value), "Shadow colors use a palette reference or hexadecimal color"),
