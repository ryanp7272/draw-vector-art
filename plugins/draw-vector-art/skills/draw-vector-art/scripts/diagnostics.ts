...
import {
  ...
  ResolutionError,
  resolveScene,
  type ResolvedDrawable,
  type ResolvedScene,
} from "./resolver.js";
import {
  EFFECT_REF_PATTERN,
  GRADIENT_REF_PATTERN,
  PALETTE_REF_PATTERN,
  SceneSchema,
  type BezierNode,
  type CompoundSubpath,
  type Drawable,
  type Paint,
  type Point,
  type ResolvedFrame,
...
function paintsFor(object: Drawable): Paint[] {
  if (object.type === "group" || object.type === "instance" || object.type === "repeater") return [];
  if (object.type === "clone") return object.paint ? [object.paint] : [];
  return [object.paint];
}

function checkPaletteRef(
  value: string,
  scene: Scene,
  pointer: string,
...
function checkEffectRef(
  ...
  if (!EFFECT_REF_PATTERN.test(value)) return;
  const name = resourceName(value);
...
function polylineSegments(points: Point[], closed: boolean): Array<[Point, Point]> {
  const segments: Array<[Point, Point]> = [];
  for (let index = 1; index < points.length; index += 1) {
    const from = points[index - 1];
    const to = points[index];
  ...
  const first = points[0];
  const last = points.at(-1);
  if (closed && first && last && !pointsEqual(first, last)) segments.push([last, first]);
  return segments;
}
...
function polylinesCross(
  ...
  return leftSegments.some((left) =>
    rightSegments.some((right) => segmentsIntersect(left[0], left[1], right[0], right[1])),
  );
}
...
function outsideLocalGrid(points: Point[]): boolean {
  const epsilon = 1e-7;
  return points.some(
    (point) => point.x < -epsilon || point.y < -epsilon || point.x > 100 + epsilon || point.y > 100 + epsilon,
  );
}

type SourceObject = Extract<Drawable, { type: "clone" | "instance" | "repeater" }>;

function isSourceObject(object: Drawable): object is SourceObject {
  return object.type === "clone" || object.type === "instance" || object.type === "repeater";
}

function isClonePrimitive(object: Drawable): boolean {
...
function referenceEdges(object: Drawable): ReferenceEdge[] {
  const edges: ReferenceEdge[] = [];
  if (object.frame.type === "relative") edges.push({ from: object.id, to: object.frame.to, kind: "frame" });
  if (object.frame.type === "mirror") edges.push({ from: object.id, to: object.frame.of, kind: "frame" });
  if (isSourceObject(object)) edges.push({ from: object.id, to: object.source, kind: "source" });
  return edges;
}

function checkReferenceCycles(
  siblingLists: Drawable[][],
  locatedById: Map<string, LocatedObject>,
  issues: ValidationIssue[],
): void {
  ...
  for (const siblings of siblingLists) {
    ...
    const visit = (id: string): void => {
      ...
      for (const edge of referenceEdges(object)) {
        ...
        if (!cycleEdges.some((entry) => entry.kind === "source")) continue;
    ...
    siblings.forEach((object) => {
      if (!state.has(object.id)) visit(object.id);
    });
  }
}
...
function expandedObjectCount(
  resolved: ResolvedDrawable,
  graph: ResolvedScene,
  throughReuse: boolean,
  trail = new Set<string>(),
): number {
  if (!resolved.visible || (resolved.object.template && !throughReuse)) return 0;
  if (trail.has(resolved.object.id)) return SCENE_EXPANSION_LIMIT + 1;
  const nextTrail = new Set(trail).add(resolved.object.id);
  const clamp = (value: number): number => Math.min(value, SCENE_EXPANSION_LIMIT + 1);

  if (resolved.object.type === "group") {
    return clamp(
      1 + resolved.children.reduce((sum, child) => sum + expandedObjectCount(child, graph, throughReuse, nextTrail), 0),
    );
  }
...
function sourceChainVisible(resolved: ResolvedDrawable, graph: ResolvedScene, trail = new Set<string>()): boolean {
...
function sourcePrimitiveObject(
  resolved: ResolvedDrawable,
  graph: ResolvedScene,
  trail = new Set<string>(),
): Drawable | undefined {
  if (trail.has(resolved.object.id)) return undefined;
  if (resolved.object.type !== "clone") return resolved.object;
  const source = graph.byId.get(resolved.object.source);
  if (!source) return undefined;
  return sourcePrimitiveObject(source, graph, new Set(trail).add(resolved.object.id));
}

function mapLocalPoint(point: Point, frame: ResolvedFrame): Point {
  return {
...
function primitiveVisualPoints(object: Drawable, frame: ResolvedFrame): Point[] {
...
function nonEmptyBounds(points: Point[]): ResolvedFrame {
...
function collectVisualSamples(graph: ResolvedScene, locatedById: Map<string, LocatedObject>): Map<string, VisualSample[]> {
  const samples = new Map<string, VisualSample[]>();
  const add = (ownerId: string, pointer: string, points: Point[], matrix: AffineMatrix): void => {
  ...
  const visit = (
    ...
    throughReuse: boolean,
    ownerOverride: { id: string; path: string } | undefined,
    trail: Set<string>,
  ): void => {
    if (!resolved.visible || (resolved.object.template && !throughReuse) || trail.has(resolved.object.id)) return;
    ...
    if (resolved.object.type === "clone") {
      const source = graph.byId.get(resolved.object.source);
      if (!source || !sourceChainVisible(source, graph)) return;
      const cloneMatrix =
        resolved.object.frame.type === "mirror"
      ...
      if (sourceObject) add(owner.id, owner.path, primitiveVisualPoints(sourceObject, resolved.frame), cloneMatrix);
      return;
    }
    if (resolved.object.type === "instance" || resolved.object.type === "repeater") {
      const source = graph.byId.get(resolved.object.source);
...
function hasComplexPaint(object: Drawable): boolean {
  return paintsFor(object).some(
    (paint) => GRADIENT_REF_PATTERN.test(paint.fill) || GRADIENT_REF_PATTERN.test(paint.stroke) || Boolean(paint.effect),
  );
}

function objectOpaqueRectangle(resolved: ResolvedDrawable, scene: Scene): boolean {
  if (
    resolved.object.type !== "rectangle" ||
    !resolved.visible ||
...
function semanticIssues(scene: Scene): { issues: ValidationIssue[]; objects: number; bezierNodes: number } {
  ...
  const generatedSvgIds = new Map<string, { path: string; description: string; objectId?: string }>();
  const reserveGeneratedId = (
    id: string,
    path: string,
    description: string,
    objectId?: string,
    ...
    if (previous) {
      issues.push(
        issue(
          ...
          `Engine-generated SVG ID ${id} for ${description} conflicts with ${previous.description}`,
          path,
          objectId,
        ),
      );
  ...
  for (const entry of located) {
    if (entry.object.type !== "group" || !entry.object.clipTo) continue;
    reserveGeneratedId(
      `clip-${entry.object.id}`,
      `${entry.path}/clipTo`,
      `the clip path for ${entry.object.id}`,
      entry.object.id,
    );
    reserveGeneratedId(
      `clip-shape-${entry.object.id}`,
      `${entry.path}/clipTo`,
      `the clip geometry for ${entry.object.id}`,
      entry.object.id,
    );
  }
    ...
    if (gradient.type === "linear" && pointsEqual(gradient.from, gradient.to)) {
      issues.push(
        issue(
          "error",
          "degenerate-gradient",
          `Linear gradient ${name} must use distinct start and end points`,
          `${gradientPath}/to`,
        ),
      );
    }
    if (gradient.type === "radial" && gradient.focal) {
      ...
      if (distance > gradient.radius) {
        issues.push(
          issue(
            "warning",
            "radial-focal-outside",
            `Radial gradient ${name} has a focal point outside its radius`,
            `${gradientPath}/focal`,
          ),
        );
      }
      ...
      if (previous && stop.offset < previous.offset) {
        issues.push(
          issue(
            "error",
            "gradient-stop-order",
            "Gradient stop offsets must be in nondecreasing order",
            `${gradientPath}/stops/${index}/offset`,
          ),
        );
      }
    });
    ...
    if (effect.dx === 0 && effect.dy === 0 && effect.blur === 0) {
      issues.push(
        issue(
          "warning",
          "ineffective-shadow",
          `Shadow effect ${name} has no offset or blur`,
          `/effects/${name}`,
        ),
      );
    }
  ...
  for (const entry of located) {
    ...
    if (reservedId) {
      issues.push(
        issue(
          "error",
          "reserved-svg-id",
          `Object ID ${object.id} conflicts with ${reservedId.description}; choose another semantic ID`,
          `${entry.path}/id`,
          object.id,
        ),
      );
    ...
    if (isSourceObject(object)) {
      ...
      if (!source) {
        ...
        issues.push(
          issue(
            "error",
            `missing-${sourceKind}-source`,
            `${sourceKind[0]?.toUpperCase() ?? "S"}${sourceKind.slice(1)} source ${object.source} must be a sibling`,
            `${entry.path}/source`,
            object.id,
          ),
        );
        ...
        if (object.type === "clone" && !isClonePrimitive(source)) {
          issues.push(
            issue(
              ...
              "Clone sources must resolve through primitives or other clones",
              `${entry.path}/source`,
              object.id,
            ),
          );
        ...
        if (!source.visible) {
          issues.push(
            issue(
              "warning",
              "invisible-source",
              `Source ${source.id} is hidden, so ${object.id} will not render its content`,
              `${entry.path}/source`,
              object.id,
            ),
          );
      ...
      if (!target) {
        ...
        issues.push(
          issue(
            "error",
            "invalid-clip-target",
            "Clip targets must resolve through drawable primitives or clones",
            `${entry.path}/clipTo`,
            object.id,
          ),
        );
    ...
    if (object.template && entry.parent?.type === "group" && entry.parent.layout.type !== "free") {
      issues.push(
        issue(
          "error",
          "template-in-layout",
          "Templates cannot occupy row or column layout slots; move the template outside the layout group",
          `${entry.path}/template`,
          object.id,
        ),
      );
      ...
      if (outsideLocalGrid(flattened)) {
        issues.push(
          issue(
            ...
            "Bézier geometry extends outside its local 0–100 frame",
            `${entry.path}/nodes`,
            object.id,
          ),
        );
    ...
    if (object.type === "compound-path") {
      const flattened: Array<{ subpath: CompoundSubpath; points: Point[]; index: number }> = [];
          ...
          if (segment.type === "arc" && pointsEqual(current, segment.to)) {
            issues.push(
              issue(
                "error",
                "degenerate-arc",
                "Arc start and endpoint must differ; use two arcs to draw a full ellipse",
                `${segmentPath}/to`,
                object.id,
              ),
            );
            ...
            if (zeroLength) {
              issues.push(
                issue(
                  "warning",
                  "zero-length-segment",
                  "Path segment has no visible length",
                  `${segmentPath}/to`,
                  object.id,
                ),
              );
        ...
        if (!subpath.closed && object.paint.fill !== "none") {
          issues.push(
            issue(
              "warning",
              "open-filled-path",
              "Open subpaths with fills are implicitly closed by SVG",
              `${entry.path}/subpaths/${subpathIndex}/closed`,
              object.id,
            ),
          );
        }
        ...
        if (hasSelfIntersection(points, subpath.closed)) {
          issues.push(
            issue(
              "error",
              "self-intersection",
              "Compound subpath intersects itself",
              `${entry.path}/subpaths/${subpathIndex}/segments`,
              object.id,
            ),
          );
        }
        const uniquePoints = points.filter(
          (point, index, all) => all.findIndex((candidate) => pointsEqual(candidate, point)) === index,
        );
        if (subpath.closed && uniquePoints.length < 3) {
          issues.push(
            issue(
              "error",
              "degenerate-subpath",
              "Closed subpaths need at least three distinct sampled points",
              `${entry.path}/subpaths/${subpathIndex}`,
              object.id,
            ),
          );
        }
      ...
      if (extendsOutsideFrame) {
        issues.push(
          issue(
            "warning",
            "geometry-outside-frame",
            "Compound path geometry extends outside its local 0–100 frame",
            `${entry.path}/subpaths`,
            object.id,
          ),
        );
      ...
      if (objectSegments > 48) {
        issues.push(
          issue(
            "warning",
            "complex-path",
            `Compound path contains ${objectSegments} segments; prefer 48 or fewer`,
            `${entry.path}/subpaths`,
            object.id,
          ),
        );
          ...
          if (
            ...
            issues.push(
              issue(
                "warning",
                "crossing-subpaths",
                `Subpath ${right} crosses subpath ${left}; verify the intended fill rule`,
                `${entry.path}/subpaths/${right}`,
                object.id,
              ),
            );
            break;
  ...
  const effectivelyReferenced = new Set<string>();
  const markReferenced = (id: string): void => {
    if (effectivelyReferenced.has(id)) return;
    effectivelyReferenced.add(id);
    const object = locatedById.get(id)?.object;
    if (!object) return;
    if (object.type === "group") object.children.forEach((child) => markReferenced(child.id));
    if (isSourceObject(object)) markReferenced(object.source);
  };
    ...
    if (entry.object.template && !effectivelyReferenced.has(entry.object.id)) {
      issues.push(
        issue(
          "warning",
          "unused-template",
          "Template is not referenced by a clone, instance, repeater, or clipping group",
          `${entry.path}/template`,
          entry.object.id,
        ),
      );
  ...
  if (compoundSegments > 256) {
    issues.push(
      issue(
        "warning",
        "excessive-segments",
        `Scene contains ${compoundSegments} compound path segments; simplify the geometry`,
        "/layers",
      ),
    );
  }
  ...
  try {
    ...
    const hasSourceCycle = issues.some((entry) =>
      ["clone-cycle", "source-cycle", "mixed-reference-cycle"].includes(entry.code),
    );
    let sceneExpansion = 0;
    if (!hasSourceCycle) {
      for (const layer of resolved.layers) {
        sceneExpansion = Math.min(
          SCENE_EXPANSION_LIMIT + 1,
          sceneExpansion + expandedObjectCount(layer, resolved, false),
        );
      }
      if (sceneExpansion > SCENE_EXPANSION_LIMIT) {
        issues.push(
          issue(
            "error",
            "excessive-expansion",
            `Scene expands to more than ${SCENE_EXPANSION_LIMIT} rendered objects`,
            "/layers",
          ),
        );
      }
      for (const item of flattenResolved(resolved.layers)) {
        if (item.object.type !== "instance" && item.object.type !== "repeater") continue;
        ...
        if (expanded > EXPANSION_LIMIT) {
          issues.push(
            issue(
              "error",
              "excessive-expansion",
              `${item.object.id} expands to more than ${EXPANSION_LIMIT} rendered objects`,
              `${entry?.path ?? "/layers"}/source`,
              item.object.id,
            ),
          );
        }
        if (item.object.type === "repeater" && item.object.count > 1) {
          ...
          if (isIdentityMatrix(step)) {
            issues.push(
              issue(
                "warning",
                "identity-repeat-step",
                "Repeater step is the identity, so every copy overlaps exactly",
                `${entry?.path ?? "/layers"}/step`,
                item.object.id,
              ),
            );
          }
