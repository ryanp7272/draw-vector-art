import type { BezierNode, Drawable, Paint, Point, ResolvedFrame, Scene } from "./schema.js";
import { flattenResolved, resolveScene, type ResolvedDrawable, type ResolvedScene } from "./resolver.js";
...
function paintAttributes(paint: Paint, scene: Scene, clip = false): string {
...
function primitiveElement(
  object: Exclude<Drawable, { type: "group" } | { type: "clone" }>,
  frame: ResolvedFrame,
  scene: Scene,
  id: string,
  paintOverride?: Paint,
...
function geometryForResolved(resolved: ResolvedDrawable, graph: ResolvedScene, id: string, clip = false): string {
  if (resolved.object.type === "group") throw new Error(`Group ${resolved.object.id} cannot be used as primitive geometry`);
...
function renderResolved(resolved: ResolvedDrawable, graph: ResolvedScene): string {
...
function clipDefinitions(graph: ResolvedScene): string[] {
  return flattenResolved(graph.layers)
    .filter((resolved) => resolved.object.type === "group" && Boolean(resolved.object.clipTo))
    .map((resolved) => {
      if (resolved.object.type !== "group" || !resolved.object.clipTo) return "";
...
function debugOverlay(graph: ResolvedScene): string {
